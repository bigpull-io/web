...
const parseIdsFromContent = (content: string, tabName: string) => {
  const [, rest] = content
    ...
    .replace(/\[tab name="[^A]*Amirdrassil Raid"/, '[tab name="Raid"')
    .replace(/\[tab name="[^S]*Season 3 Mythic\+"/, '[tab name="Mythic+"')
...
(async () => {
  ...
  await Promise.all(
    urls
      ...
      .map(({ specId, url }) =>
        queue.add(async () => {
          console.log('[M+ S3 Loot] Fetching', url);
  ...
  console.log('[M+ S3 BIS] Writing to file...');

  await writeFile('./src/data/mplus-s3-bis.json', JSON.stringify(bis));

  console.log('[M+ S3 BIS] Done!');
})();
